...
ScreenGui.Parent = game:GetService("CoreGui") -- Parent it immediately

-- Window size definitions
local windowExpandedSize = UDim2.new(0, 400, 0, 450)
local windowCollapsedSize = UDim2.new(0, 400, 0, 30)

local MainFrame = Instance.new("Frame")
MainFrame.Name = "MainFrame"
MainFrame.Size = windowExpandedSize
...
local PagesScrollingFrame = Instance.new("ScrollingFrame")
PagesScrollingFrame.Name = "PagesScrollingFrame"
PagesScrollingFrame.Size = UDim2.new(1, -10, 1, -70)
PagesScrollingFrame.Position = UDim2.new(0, 5, 0, 65)
PagesScrollingFrame.BackgroundTransparency = 1
PagesScrollingFrame.ScrollBarThickness = 5
PagesScrollingFrame.ScrollBarImageColor3 = colors.Border
PagesScrollingFrame.ScrollingDirection = Enum.ScrollingDirection.Y
PagesScrollingFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
...
PagesHolder.Parent = PagesScrollingFrame

local PagesListLayout = Instance.new("UIListLayout")
PagesListLayout.Name = "PagesListLayout"
PagesListLayout.Padding = UDim.new(0, 5)
PagesListLayout.Parent = PagesHolder
...
PagesScrollingFrame.Parent = MainFrame
...
if UserInputService.TouchEnabled then
    windowExpandedSize = UDim2.new(0, 350, 0, 400)
    windowCollapsedSize = UDim2.new(0, 350, 0, 30)
    MainFrame.Size = windowExpandedSize
...
function OrionLib:Tab(name)
    if not name then error("Tab name cannot be nil") end
    ...
    function tab:Section(name)
        if not name then error("Section name cannot be nil") end
        ...
        ContentCorner.Parent = SectionContent
        ...
        function section:Button(name, callback, description)
            if not name then error("Button name cannot be nil") end
        ...
        function section:Slider(name, min, max, default, callback, description)
            ...
            -- Changed from Frame to TextButton for the slider button
            ...
            SliderButton.Text = ""
            SliderButton.AutoButtonColor = false -- Important to prevent button color changes
            SliderButton.Parent = SliderBar
            
            -- Add a transparent button over the entire slider bar for clicking
            local SliderBarButton = Instance.new("TextButton")
            SliderBarButton.Name = "BarButton"
            ...
            SliderBarButton.MouseButton1Down:Connect(function(x, y)
